...
The PowerShell scripts included in this folder are the commands used during the episode to verify and troubleshoot BitLocker.

### Get-BitLockerStatus.ps1

Displays the current BitLocker state of the operating system drive, including:

- Volume status
- Protection status
- Encryption percentage
- Encryption method
- Key protector status

### Get-BitLockerEvents.ps1

Displays recent events from the Windows BitLocker Management event log.

This was used to identify Event ID 853 and the bootable media that was preventing silent BitLocker encryption.

### Test-BitLockerPrerequisites.ps1

Checks the main prerequisites we investigated during troubleshooting:
...
Decrypts the operating system volume.

This was used in the lab to return LAB-PC01 to a clean state before allowing the Intune policy to configure BitLocker again.

WARNING: This script decrypts the operating system drive. Review and understand the impact before using it.

## Important

The configuration and scripts in this repository are intended for demonstration and lab use.
